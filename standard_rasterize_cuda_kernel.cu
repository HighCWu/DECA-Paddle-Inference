...
__device__ __forceinline__ bool check_face_frontside(const data_t *face) {
    return (face[7] - face[1]) * (face[3] - face[0]) < (face[4] - face[1]) * (face[6] - face[0]);
}


template <typename data_t> struct point
{
    public:
    data_t x;
    data_t y;

    __host__ __device__ data_t dot(point<data_t> p)
    {
        return this->x * p.x + this->y * p.y;
    };

    __host__ __device__  point<data_t> operator-(point<data_t>& p)
    {
        point<data_t> np;
        np.x = this->x - p.x;
        np.y = this->y - p.y;
    ...
    __host__ __device__  point<data_t> operator*(data_t s)
    {
        point<data_t> np;
        np.x = s * this->x;
        np.y = s * this->y;
...
template <typename data_t>
__device__ __forceinline__ bool check_pixel_inside(const data_t *w) {
    return w[0] <= 1 && w[0] >= 0 && w[1] <= 1 && w[1] >= 0 && w[2] <= 1 && w[2] >= 0;
}

template <typename data_t>
__device__ __forceinline__ void barycentric_weight(data_t *w,  point<data_t> p, point<data_t> p0,  point<data_t> p1,  point<data_t> p2) {
    
    // vectors
    point<data_t> v0, v1, v2;
    data_t s = p.dot(p);
    v0 = p2 - p0; 
    v1 = p1 - p0; 
    v2 = p - p0; 
    ...
    // dot products
    data_t dot00 = v0.dot(v0); //v0.x * v0.x + v0.y * v0.y //np.dot(v0.T, v0)
    data_t dot01 = v0.dot(v1); //v0.x * v1.x + v0.y * v1.y //np.dot(v0.T, v1)
    data_t dot02 = v0.dot(v2); //v0.x * v2.x + v0.y * v2.y //np.dot(v0.T, v2)
    data_t dot11 = v1.dot(v1); //v1.x * v1.x + v1.y * v1.y //np.dot(v1.T, v1)
    data_t dot12 = v1.dot(v2); //v1.x * v2.x + v1.y * v2.y//np.dot(v1.T, v2)
    ...
    data_t u = (dot11*dot02 - dot01*dot12)*inverDeno;
    data_t v = (dot00*dot12 - dot01*dot02)*inverDeno;
...
__global__ void forward_rasterize_cuda_kernel(
        const data_t* __restrict__ face_vertices, //[bz, nf, 3, 3]
        data_t*  depth_buffer,
        int*  triangle_buffer,
        data_t*  baryw_buffer,        
    ...
    int bn = i/ntri;
    const data_t* face = &face_vertices[i * 9];
    data_t bw[3];
    point<data_t> p0, p1, p2, p;
    ...
    for(int y = y_min; y <= y_max; y++) //h
    {
        for(int x = x_min; x <= x_max; x++) //w
        {
            ...
            if((bw[2] >= 0) && (bw[1] >= 0) && (bw[0]>0))
            {
                ...
                data_t zp = 1. / (bw[0] / face[2] + bw[1] / face[5] + bw[2] / face[8]);
...
__global__ void forward_rasterize_colors_cuda_kernel(
        const data_t* __restrict__ face_vertices, //[bz, nf, 3, 3]
        const data_t* __restrict__ face_colors, //[bz, nf, 3, 3]
        data_t*  depth_buffer,
        int*  triangle_buffer,
        data_t*  images,        
        int batch_size, int h, int w, 
    ...
    int bn = i/ntri;
    const data_t* face = &face_vertices[i * 9];
    const data_t* color = &face_colors[i * 9];
    data_t bw[3];
    point<data_t> p0, p1, p2, p;
...
std::vector<paddle::Tensor> forward_rasterize_cuda(
    ...
    PD_DISPATCH_FLOATING_TYPES(face_vertices.type(), "forward_rasterize_cuda1", ([&] {
      forward_rasterize_cuda_kernel<data_t><<<blocks_1, threads>>>(
        ...
        const_cast<data_t*>(depth_buffer.data<data_t>()),
        const_cast<int*>(triangle_buffer.data<int>()),
        const_cast<data_t*>(baryw_buffer.data<data_t>()),
        batch_size, h, w,
        ntri);
      }));
...
std::vector<paddle::Tensor> forward_rasterize_colors_cuda(
    ...
    PD_DISPATCH_FLOATING_TYPES(face_vertices.type(), "forward_rasterize_colors_cuda", ([&] {
      forward_rasterize_colors_cuda_kernel<data_t><<<blocks_1, threads>>>(
        ...
        const_cast<data_t*>(depth_buffer.data<data_t>()),
        const_cast<int*>(triangle_buffer.data<int>()),
        const_cast<data_t*>(images.data<data_t>()),
        batch_size, h, w,
        ntri);
      }));
